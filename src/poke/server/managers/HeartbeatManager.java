...
import poke.core.Mgmt.logReplication;
...
public class HeartbeatManager extends Thread {
	...
	public void processRequest(Management mgmt) {
		...
		if (hd == null) {
			...
			/*
			 * Mario's comment: If HB from leader, restart Timer!
			 */
			//System.out.println("hd.getNodeId():"+hd.getNodeId());
			//System.out.println("ElectionManager.getInstance().getLeaderNode():"+ElectionManager.getInstance().getLeaderNode());
			
			if(ElectionManager.getInstance().getLeaderNode()!=null && hd.getNodeId() == ElectionManager.getInstance().getLeaderNode()) {
				TimeoutManager.getInstance().reInitializeByHB();
				//Add code to sync the list of cluster leader nodes from leader.
					
				ClusterManager.getInstance().resetRemoteClusterLeaders();
				for(logReplication lR :req.getLogsList()) {
					ClusterManager.getInstance().addRemoteClusterLeader(lR.getClusterId(), lR.getLeaderNodeId());
	...
	private Management generateHB() {
		...
		for (Map.Entry<Integer, Integer> entry : ClusterManager.getInstance().getRemoteClusterLeaders().entrySet()) {
		    logReplication.Builder lRB = logReplication.newBuilder();
		    lRB.setClusterId(entry.getKey());
		    lRB.setLeaderNodeId(entry.getValue());
			h.addLogs(lRB);
		}		
	...
	public class CloseHeartListener implements ChannelFutureListener {
		...
		public void operationComplete(ChannelFuture future) throws Exception {
			if (outgoingHB.containsValue(heart)) {
				...
				if(ElectionManager.getInstance().getLeaderNode() == heart.getNodeId()) {
                    ...
                    ElectionManager.getInstance().setLeaderNode();
					ElectionManager.getInstance().setFirstTime();
