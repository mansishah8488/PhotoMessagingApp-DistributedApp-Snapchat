...
public class RAFTElection implements Election {
	...
	public boolean updateCurrent(LeaderElection req) {
		...
		/*
		 * Mario's comment: Added below clause to avoid havin an issue of 2 nodes starting an election
		 * Leading to a scenario where each node thinks the other is a leader.
		 * By checking the timestamp, we can ensure only the node that started the election first can
		 * conduct it.
		 */
		else {
	...
	private synchronized Management castVote(Management mgmt, boolean isNew) {
		...
		/** Mario's comment: This block of code need not be present since each node will not 
		...
		if (req.getCandidateId() == this.nodeId) {
			...
			/** Mario's comment: In this case we need to count the no of votes and 
			...
			System.out.println("I am here without hops finally");
		}
