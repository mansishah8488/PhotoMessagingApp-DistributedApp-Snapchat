...
import poke.server.cluster.ClusterInitializer;
...
import poke.server.managers.TimeoutManager;
...
public class Server {
	...
	protected TimeoutManager timeoutMgr;
	...
	private static class StartManagement implements Runnable {
		...
		public void run() {
			...
			try {
				...
				//b.childHandler(new ClusterInitializer(compressComm));
	...
	private void startManagers() {
		...
		/*
		 * Mario's comment: Added code below to initialize timeout manager!
		 */
		timeoutMgr = TimeoutManager.initialize();
		Thread t = new Thread(timeoutMgr);
		t.start();
		
		clusterMgr = ClusterManager.initialize(conf);
		Thread t2 = new Thread(clusterMgr);
		t2.start();
